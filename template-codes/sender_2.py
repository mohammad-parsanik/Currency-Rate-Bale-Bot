...
MESSAGE_TEXT = """#کارنامه | با سلام و احترام 

از هم اکنون می‌توانید کارنامه دوره پاییز دانش‌آموز خود را [از این نشانی]({link}) دریافت کنید.

*مرکز علمی پژوهشی ژرفا | سمپاد تهران*"""
...
def send_message(phone_number: str, link: str) -> bool:
    ...
    payload = {
        ...
        "message_data": {
            "message": {
                "text": MESSAGE_TEXT.format(link = link),
                #"file_id": file_id
            }
        }
    }
...
def process_excel(file_path: str):
    ...
    try:
        ...
        df.columns = ['Phone1', 'Phone2', 'Link'] 
        
        if not all(col in df.columns for col in ['Phone1', 'Phone2', 'Link']):
            logging.error("Excel columns do not match expected structure: Phone1, Phone2, Link.")
